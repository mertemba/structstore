...
public:
    ...
    StructStoreField& get_field(HashString name) {
        ...
        if (it == fields.end()) {
            ...
            auto ret = fields.emplace(name_int, StructStoreField{});
            if (!ret.second) {
                throw std::runtime_error("internal error in StructStore::get_field()");
            }
            it = ret.first;
    ...
    FieldAccess operator[](HashString name) {
        StructStoreField& field = get_field(name);
        return {field, mm_alloc, unmanaged};
    }
